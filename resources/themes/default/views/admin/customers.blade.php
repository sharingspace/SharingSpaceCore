        ...
        data-cookie-id-table="customers">
          ...
          <tbody>
            ...
            <tr {!! ($subscription->active!='1' ? 'class="danger"' : '') !!}>
              ...
              <td>{{ $subscription_meta->interval }}</td>
              <td>{{ $subscription->created_at->format('M d, Y') }}</td>
              <td>{{ ($subscription->trial_starts_at!='') ? date('M d, Y',strtotime($subscription->trial_starts_at)) : ''}}</td>
              <td>{{ ($subscription->trial_ends_at!='') ? date('M d, Y',strtotime($subscription->trial_ends_at)) : ''}}</td>
              <td>{{ ($subscription->period_ends_at!='') ? date('M d, Y',strtotime($subscription->period_ends_at)) : ''}}</td>
            </tr>
            @endforeach
          </tbody>
        </table>
